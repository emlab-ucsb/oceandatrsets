#The GFW anchorages data can be downloaded from the GFW data page (login
#required, but is free):
#https://globalfishingwatch.org/data-download/datasets/public-anchorages This
#datasets is big (~167, 000 rows) and finding distance from each of these points
#to cells in a grid will take too long on and too much memory for most
#computers. GFW identifies anchorages as anywhere vessels with AIS remain
#stationary for 12 hours or more, which can lead to many anchorages very close
#to each other, e.g. outside ports where vessels wait for docks to become
#available. Anchorages close together have the same names, so to reduce the
#number of anchorages, they are aggregated by iso3 code (country code) and label
#(name) and the mean longitude and latitude coordinates obtained to get one
#anchorage point per name in each country. To further reduce the number of
#points, anchorages within countries' land boundaries, e.g. along rivers, can be
#removed. I do this by buffering the Natural Earth land boundaries by 10km
#inland so as to avoid cutting off coastal anchorages that fall within the land
#boundary, due to inaccuracies in the Natural Earth land boundaries, e.g. for
#islands and other small scale coastlines, and then masking points that fall
#within the resulting polygons.

#load the raw anchorages csv and save as rds to compress
# data downloaded from GFW 2026-5-20
path_to_gfw_anchorages <- "data-raw/named_anchorages_v2_pipe_v3_202601.csv"

anchorages_all <- read.csv(path_to_gfw_anchorages) |>
  subset(!(lat < -90 | lat > 90 | lon < -180 | lon > 180)) #there is a point at longitude = 1001!

library(sf) #have to load this for rnaturalearth call to work
...
terra::plet(terra::crop(anchorages_name_grouped_masked, baltic_ext), "on_land", cex = 0.5) |>
  terra::points(terra::crop(anchorages_all, baltic_ext), col = "forestgreen", alpha = 1, cex =2) |>
  terra::lines(terra::crop(natural_earth_high_res, baltic_ext), "brown") |>
  terra::lines(terra::crop(natural_earth_high_res_shrunk, baltic_ext), "pink")
  ...
  #this checks that no anchorages with the same label (name assigned to it) cross the antimeridian - they don't. So we can safely group anchorages by label and
   read.csv(path_to_gfw_anchorages) |>
    subset(!(lat < -90 | lat > 90 | lon < -180 | lon > 180)) |>  #there is a point at longitude = 1001!
